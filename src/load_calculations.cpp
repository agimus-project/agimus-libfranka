...
std::array<double, 3> combineCenterOfMass(
    ...
    const std::array<double, 3>&
        F_x_Cee,  // NOLINT(readability-identifier-naming)
    double m_load,
    const std::array<double, 3>&
  ...
  if ((m_ee + m_load) > 0) {
    for (size_t i = 0; i < F_x_Ctotal.size(); i++) {
      F_x_Ctotal[i] =
          (m_ee * F_x_Cee[i] + m_load * F_x_Cload[i]) / (m_ee + m_load);
    }
  }
...
Eigen::Matrix3d skewSymmetricMatrixFromVector(const Eigen::Vector3d& input) {
  ...
  input_hat << 0, -input(2), input(1), input(2), 0, -input(0), -input(1),
      input(0), 0;
  return input_hat;
}
...
std::array<double, 9> combineInertiaTensor(
    ...
    const std::array<double, 9>&
        I_load,  // NOLINT(readability-identifier-naming)
    double m_total,
    const std::array<double, 3>&
