...
ControlLoop<T>::ControlLoop(RobotControl& robot,
  ...
  if (throw_on_error && !hasRealtimeKernel()) {
    throw RealtimeException(
        "libagimus_franka: Running kernel does not have realtime "
        "capabilities.");
  }
}
...
ControlLoop<T>::ControlLoop(RobotControl& robot,
    ...
    : ControlLoop(robot, std::move(motion_callback),
                  std::move(control_callback), limit_rate, cutoff_frequency) {
  if (!control_callback_) {
    throw std::invalid_argument(
        "libagimus_franka: Invalid control callback given.");
  }
  if (!motion_callback_) {
    throw std::invalid_argument(
        "libagimus_franka: Invalid motion callback given.");
  }

  motion_id_ = robot.startMotion(agimus_research_interface::robot::Move::
...
template <typename T>
ControlLoop<T>::ControlLoop(RobotControl& robot, ControllerMode controller_mode,
                            MotionGeneratorCallback motion_callback,
                            bool limit_rate, double cutoff_frequency)
  ...
  switch (controller_mode) {
    ...
    default:
      throw std::invalid_argument(
          "libagimus_franka: Invalid controller mode given.");
  }
  motion_id_ =
...
void ControlLoop<T>::operator()() try {
  ...
  if (control_callback_) {
    ...
    while (spinMotion(robot_state, robot_state.time - previous_time,
                      &motion_command) &&
           spinControl(robot_state, robot_state.time - previous_time,
                       &control_command)) {
...
template <typename T>
bool ControlLoop<T>::spinControl(
  ...
  if (cutoff_frequency_ < kMaxCutoffFrequency) {
    for (size_t i = 0; i < 7; i++) {
      control_output.tau_J[i] =
          lowpassFilter(kDeltaT, control_output.tau_J[i],
                        robot_state.tau_J_d[i], cutoff_frequency_);
    }
  }
  if (limit_rate_) {
    control_output.tau_J =
        limitRate(kMaxTorqueRate, control_output.tau_J, robot_state.tau_J_d);
  }
...
void ControlLoop<JointPositions>::convertMotion(
    const JointPositions& motion, const RobotState& robot_state,
    agimus_research_interface::robot::MotionGeneratorCommand* command) {
  command->q_c = motion.q;
  if (cutoff_frequency_ < kMaxCutoffFrequency) {
    for (size_t i = 0; i < 7; i++) {
      command->q_c[i] = lowpassFilter(kDeltaT, command->q_c[i],
                                      robot_state.q_d[i], cutoff_frequency_);
    }
  }
  if (limit_rate_) {
    command->q_c =
...
void ControlLoop<JointVelocities>::convertMotion(
    const JointVelocities& motion, const RobotState& robot_state,
    agimus_research_interface::robot::MotionGeneratorCommand* command) {
  command->dq_c = motion.dq;
  if (cutoff_frequency_ < kMaxCutoffFrequency) {
    for (size_t i = 0; i < 7; i++) {
      command->dq_c[i] = lowpassFilter(kDeltaT, command->dq_c[i],
                                       robot_state.dq_d[i], cutoff_frequency_);
    }
  }
  if (limit_rate_) {
    command->dq_c = limitRate(
        computeUpperLimitsJointVelocity(robot_state.q_d),
...
void ControlLoop<CartesianPose>::convertMotion(
    const CartesianPose& motion, const RobotState& robot_state,
    agimus_research_interface::robot::MotionGeneratorCommand* command) {
  command->O_T_EE_c = motion.O_T_EE;
  if (cutoff_frequency_ < kMaxCutoffFrequency) {
    command->O_T_EE_c = cartesianLowpassFilter(
        kDeltaT, command->O_T_EE_c, robot_state.O_T_EE_c, cutoff_frequency_);
  }

  if (limit_rate_) {
    command->O_T_EE_c = limitRate(
        kMaxTranslationalVelocity, kMaxTranslationalAcceleration,
        kMaxTranslationalJerk, kMaxRotationalVelocity,
        kMaxRotationalAcceleration, kMaxRotationalJerk, command->O_T_EE_c,
  ...
  if (motion.hasElbow()) {
    ...
    if (cutoff_frequency_ < kMaxCutoffFrequency) {
      command->elbow_c[0] =
          lowpassFilter(kDeltaT, command->elbow_c[0], robot_state.elbow_c[0],
                        cutoff_frequency_);
    }
    if (limit_rate_) {
      command->elbow_c[0] =
          limitRate(kMaxElbowVelocity, kMaxElbowAcceleration, kMaxElbowJerk,
                    command->elbow_c[0], robot_state.elbow_c[0],
...
void ControlLoop<CartesianVelocities>::convertMotion(
    const CartesianVelocities& motion, const RobotState& robot_state,
    agimus_research_interface::robot::MotionGeneratorCommand* command) {
  command->O_dP_EE_c = motion.O_dP_EE;
  if (cutoff_frequency_ < kMaxCutoffFrequency) {
    for (size_t i = 0; i < 6; i++) {
      command->O_dP_EE_c[i] =
          lowpassFilter(kDeltaT, command->O_dP_EE_c[i],
  ...
  if (limit_rate_) {
    command->O_dP_EE_c = limitRate(
        kMaxTranslationalVelocity, kMaxTranslationalAcceleration,
        kMaxTranslationalJerk, kMaxRotationalVelocity,
        kMaxRotationalAcceleration, kMaxRotationalJerk, command->O_dP_EE_c,
