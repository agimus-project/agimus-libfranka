...
Eigen::Vector3d limitRate(double max_velocity, double max_acceleration,
                          double max_jerk,
  ...
  Eigen::Vector3d commanded_jerk =
      (((commanded_velocity - last_commanded_velocity) / kDeltaT) -
  ...
  if (commanded_jerk.norm() > kNormEps) {
    commanded_acceleration +=
        (commanded_jerk / commanded_jerk.norm()) *
        std::max(std::min(commanded_jerk.norm(), max_jerk), -max_jerk) *
        kDeltaT;
  }

  // Compute Euclidean distance to the max velocity vector that would be reached
  // starting from last_commanded_velocity with the direction of the desired
  // acceleration
  Eigen::Vector3d unit_commanded_acceleration =
      commanded_acceleration / commanded_acceleration.norm();
  double dot_product =
      unit_commanded_acceleration.transpose() * last_commanded_velocity;
  double distance_to_max_velocity =
      -dot_product +
      std::sqrt(pow(dot_product, 2.0) - last_commanded_velocity.squaredNorm() +
                pow(max_velocity, 2.0));

  // Compute safe acceleration limits
  double safe_max_acceleration =
      std::min((max_jerk / max_acceleration) * distance_to_max_velocity,
               max_acceleration);
...
std::array<double, 7> limitRate(
  ...
  for (size_t i = 0; i < 7; i++) {
    double commanded_derivative =
        (commanded_values[i] - last_commanded_values[i]) / kDeltaT;
    limited_values[i] =
        last_commanded_values[i] +
        std::max(std::min(commanded_derivative, max_derivatives[i]),
                 -max_derivatives[i]) *
            kDeltaT;
  }
...
double limitRate(double upper_limits_velocity, double lower_limits_velocity,
  ...
  double safe_max_acceleration =
      std::min((max_jerk / max_acceleration) *
                   (upper_limits_velocity - last_commanded_velocity),
               max_acceleration);
  double safe_min_acceleration =
      std::max((max_jerk / max_acceleration) *
                   (lower_limits_velocity - last_commanded_velocity),
               -max_acceleration);
...
double limitRate(double upper_limits_velocity, double lower_limits_velocity,
  ...
  return last_commanded_position +
         limitRate(upper_limits_velocity, lower_limits_velocity,
...
std::array<double, 7> limitRate(
  ...
  for (size_t i = 0; i < 7; i++) {
    limited_commanded_velocities[i] = limitRate(
        upper_limits_velocity[i], lower_limits_velocity[i], max_acceleration[i],
        max_jerk[i], commanded_velocities[i], last_commanded_velocities[i],
...
std::array<double, 6> limitRate(
  ...
  dx.head(3) << limitRate(
      max_translational_velocity, max_translational_acceleration,
      max_translational_jerk, dx.head(3), last_dx.head(3), last_ddx.head(3));
  dx.tail(3) << limitRate(max_rotational_velocity, max_rotational_acceleration,
                          max_rotational_jerk, dx.tail(3), last_dx.tail(3),
                          last_ddx.tail(3));
...
std::array<double, 16> limitRate(
  ...
  Eigen::Affine3d last_commanded_pose(
      Eigen::Matrix4d::Map(last_O_T_EE_c.data()));

  // Compute translational velocity
  dx.head(3) << (commanded_pose.translation() -
                 last_commanded_pose.translation()) /
                    kDeltaT;
  ...
  std::array<double, 6>
      commanded_O_dP_EE_c{};  // NOLINT(readability-identifier-naming)
  Eigen::Map<Eigen::Matrix<double, 6, 1>>(&commanded_O_dP_EE_c[0], 6, 1) = dx;
  commanded_O_dP_EE_c = limitRate(
      max_translational_velocity, max_translational_acceleration,
  ...
  limited_commanded_pose.translation()
      << last_commanded_pose.translation() + dx.head(3) * kDeltaT;
  ...
  if (dx.tail(3).norm() > kNormEps) {
    ...
    omega_skew << 0, -w_norm(2), w_norm(1), w_norm(2), 0, -w_norm(0),
        -w_norm(1), w_norm(0), 0;
  ...
  Eigen::Map<Eigen::Matrix4d>(&limited_values[0], 4, 4) =
      limited_commanded_pose.matrix();
  return limited_values;
}
