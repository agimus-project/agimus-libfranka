 ...
 * state, gives access to the model library and allows to control the robot.
 *
 * @note
 ...
 * @anchor o-frame
 * @par Base frame O
 * The base frame is located at the center of the robot's base. Its z-axis is
 * identical with the axis of rotation of the first joint.
 *
 * @anchor f-frame
 * @par Flange frame F
 * The flange frame is located at the center of the flange surface. Its z-axis
 * is identical with the axis of rotation of the last joint. This frame is fixed
 * and cannot be changed.
 *
 * @anchor ne-frame
 * @par Nominal end effector frame NE
 * The nominal end effector frame is configured outside of libagimus_franka (in
 * DESK) and cannot be changed here. It may be used to set end effector frames
 * which are rarely changed.
 *
 * @anchor ee-frame
 * @par end effector frame EE
 * By default, the end effector frame EE is the same as the nominal end effector
 * frame NE (i.e. the transformation between NE and EE is the identity
 * transformation). It may be used to set end effector frames which are changed
 * more frequently (such as a tool that is grasped with the end effector). With
 * Robot::setEE, a custom transformation matrix can be set.
 *
 * @anchor k-frame
 * @par Stiffness frame K
 * This frame describes the transformation from the end effector frame EE to the
 * stiffness frame K. The stiffness frame is used for Cartesian impedance
 * control, and for measuring and applying forces. The values set using
 * Robot::setCartesianImpedance are used in the direction of the stiffness
 * frame. It can be set with Robot::setK. This frame allows to modify the
 * compliance behavior of the robot (e.g. to have a low stiffness around a
 * specific point which is not the end effector). The stiffness frame does not
 * affect where the robot will move to. The user should always command the
 * desired end effector frame (and not the desired stiffness frame).
 */
class Robot {
 public:
  /**
   ...
   * @param[in] log_size sets how many last states should be kept for logging
   * purposes. The log is provided when a ControlException is thrown.
   *
   ...
   *
   * The following methods allow to perform motion generation and/or send
   * joint-level torque commands without gravity and friction by providing
   * callback functions.
   *
   * Only one of these methods can be active at the same time; a
   * agimus_franka::ControlException is thrown otherwise.
   *
   ...
   * * A agimus_franka::Duration to indicate the time since the last callback
   * invocation. Thus, the duration is zero on the first invocation of the
   * callback function!
   *
   * The following incomplete example shows the general structure of a callback
   * function:
   *
   * @code{.cpp}
   * double time = 0.0;
   * auto control_callback = [&time](const agimus_franka::RobotState&
   * robot_state, agimus_franka::Duration time_step) ->
   * agimus_franka::JointPositions { time += time_step.toSec();  // Update time
   ...
   * Cannot be executed while another control or motion generator loop is
   * active.
   *
   * @param[in] control_callback Callback function providing joint-level torque
   * commands. See @ref callback-docs "here" for more details.
   * @param[in] limit_rate True if rate limiting should be activated. False by
   * default. This could distort your motion!
   * @param[in] cutoff_frequency Cutoff frequency for a first order low-pass
   * filter applied on the user commanded signal. Set to
   * agimus_franka::kMaxCutoffFrequency to disable.
   *
   * @throw ControlException if an error related to torque control or motion
   * generation occurred.
   * @throw InvalidOperationException if a conflicting operation is already
   * running.
   * @throw NetworkException if the connection is lost, e.g. after a timeout.
   * @throw RealtimeException if realtime priority cannot be set for the current
   * thread.
   * @throw std::invalid_argument if joint-level torque commands are NaN or
   * infinity.
   *
  ...
  /**
   * Starts a control loop for sending joint-level torque commands and joint
   * positions.
   *
   * Sets realtime priority for the current thread.
   * Cannot be executed while another control or motion generator loop is
   ...
   * generation. See @ref callback-docs "here" for more details.
   * @param[in] limit_rate True if rate limiting should be activated. False by
   * default. This could distort your motion!
   * @param[in] cutoff_frequency Cutoff frequency for a first order low-pass
   * filter applied on the user commanded signal. Set to
   ...
   * @throw RealtimeException if realtime priority cannot be set for the current
   * thread.
   * @throw std::invalid_argument if joint-level torque or joint position
   * commands are NaN or infinity.
   *
   * @see Robot::Robot to change behavior if realtime priority cannot be set.
   */
  void control(
   ...
   * @throw RealtimeException if realtime priority cannot be set for the current
   * thread.
   * @throw std::invalid_argument if joint-level torque or joint velocity
   * commands are NaN or infinity.
   *
   * @see Robot::Robot to change behavior if realtime priority cannot be set.
   */
  ...
  /**
   * Starts a control loop for sending joint-level torque commands and Cartesian
   * poses.
   *
   * Sets realtime priority for the current thread.
   * Cannot be executed while another control or motion generator loop is
   ...
   * @throw RealtimeException if realtime priority cannot be set for the current
   * thread.
   * @throw std::invalid_argument if joint-level torque or Cartesian pose
   * command elements are NaN or infinity.
   *
   * @see Robot::Robot to change behavior if realtime priority cannot be set.
   */
  void control(
   ...
   * @throw RealtimeException if realtime priority cannot be set for the current
   * thread.
   * @throw std::invalid_argument if joint-level torque or Cartesian velocity
   * command elements are NaN or infinity.
   *
   * @see Robot::Robot to change behavior if realtime priority cannot be set.
   */
   ...
   * @throw RealtimeException if realtime priority cannot be set for the current
   * thread.
   * @throw std::invalid_argument if joint position commands are NaN or
   * infinity.
   *
   * @see Robot::Robot to change behavior if realtime priority cannot be set.
   */
   ...
   * @throw RealtimeException if realtime priority cannot be set for the current
   * thread.
   * @throw std::invalid_argument if joint velocity commands are NaN or
   * infinity.
   *
   * @see Robot::Robot to change behavior if realtime priority cannot be set.
   */
   ...
   * @throw RealtimeException if realtime priority cannot be set for the current
   * thread.
   * @throw std::invalid_argument if Cartesian pose command elements are NaN or
   * infinity.
   *
   * @see Robot::Robot to change behavior if realtime priority cannot be set.
   */
  ...
  /**
   * Starts a control loop for a Cartesian velocity motion generator with a
   ...
   * @throw RealtimeException if realtime priority cannot be set for the current
   * thread.
   * @throw std::invalid_argument if Cartesian velocity command elements are NaN
   * or infinity.
   *
   * @see Robot::Robot to change behavior if realtime priority cannot be set.
   */
   ...
   * during acceleration/deceleration for each joint in \f$[Nm]\f$.
   * @param[in] upper_torque_thresholds_acceleration Collision torque thresholds
   * during acceleration/deceleration for each joint in \f$[Nm]\f$.
   * @param[in] lower_torque_thresholds_nominal Contact torque thresholds for
   * each joint in \f$[Nm]\f$.
   * @param[in] upper_torque_thresholds_nominal Collision torque thresholds for
   * each joint in \f$[Nm]\f$.
   * @param[in] lower_force_thresholds_acceleration Contact force thresholds
   * during acceleration/deceleration for \f$(x,y,z,R,P,Y)\f$ in \f$[N]\f$.
   * @param[in] upper_force_thresholds_acceleration Collision force thresholds
   * during acceleration/deceleration for \f$(x,y,z,R,P,Y)\f$ in \f$[N]\f$.
   * @param[in] lower_force_thresholds_nominal Contact force thresholds for
   * \f$(x,y,z,R,P,Y)\f$ in \f$[N]\f$.
   * @param[in] upper_force_thresholds_nominal Collision force thresholds for
   * \f$(x,y,z,R,P,Y)\f$ in \f$[N]\f$.
   *
   ...
   * registered as collision and cause the robot to stop moving.
   *
   * @param[in] lower_torque_thresholds Contact torque thresholds for each joint
   * in \f$[Nm]\f$.
   * @param[in] upper_torque_thresholds Collision torque thresholds for each
   * joint in \f$[Nm]\f$.
   * @param[in] lower_force_thresholds Contact force thresholds for
   * \f$(x,y,z,R,P,Y)\f$ in \f$[N]\f$.
   * @param[in] upper_force_thresholds Collision force thresholds for
   * \f$(x,y,z,R,P,Y)\f$ in \f$[N]\f$.
   *
   * @throw CommandException if the Control reports an error.
   * @throw NetworkException if the connection is lost, e.g. after a timeout.
   ...
   *
   * @param[in] K_theta Joint impedance values \f$K_{\theta_{1-7}} = \in
   * [0,14250]
   * \frac{Nm}{rad}\f$
   *
   * @throw CommandException if the Control reports an error.
  ...
  /**
   * Sets the Cartesian stiffness/compliance (for x, y, z, roll, pitch, yaw) in
   * the internal controller.
   *
   * The values set using Robot::setCartesianImpedance are used in the direction
   ...
   *
   * @param[in] K_x Cartesian impedance values \f$K_x=(K_{x_{x,y,z}} \in
   * [10,3000] \frac{N}{m}, K_{x_{R,P,Y}} \in [1,300] \frac{Nm}{rad})\f$
   * @throw CommandException if the Control reports an error.
   * @throw NetworkException if the connection is lost, e.g. after a timeout.
   */
   ...
   *
   * @note
   * Guiding mode can be enabled by pressing the two opposing buttons near the
   * robot's flange.
   *
   ...
   * column-major format.
   *
   * @param[in] NE_T_EE Vectorized NE-to-EE transformation matrix
   * \f$^{NE}T_{EE}\f$, column-major.
   *
   * @throw CommandException if the Control reports an error.
   * @throw NetworkException if the connection is lost, e.g. after a timeout.
   *
   * @see RobotState::NE_T_EE for end effector pose in @ref ne-frame "nominal
   * end effector frame NE".
   * @see RobotState::O_T_EE for end effector pose in @ref o-frame "world base
   * frame O".
   * @see RobotState::F_T_EE for end effector pose in @ref f-frame "flange frame
   * F".
   */
   ...
   *
   * @param control_type agimus_research_interface::robot::Move::ControllerMode
   * control type for the operation
   ...
   *
   * If a control or motion generator loop is running in another thread, it will
   * be preempted with a agimus_franka::ControlException.
   *
   * @throw CommandException if the Control reports an error.
   * @throw NetworkException if the connection is lost, e.g. after a timeout.
   */
 ...
 protected:
  /**
   * Constructs a new Robot given a Robot::Impl. This enables unittests with
   * Robot::Impl-Mocks.
   *
 ...
 private:
   ...
   * @param control_mode defines the type of motion / control that shall be
   started
