 ...
 * Contains functions for limiting the rate of torques, Cartesian pose,
 * Cartesian velocity, joint position and joint velocity.
 */
 ...
 * For FR3 there are no expected package loses. Therefore this number is set to
 * 0. If you encounter package loses with your setup you can increase this
 * number
 */
constexpr double kTolNumberPacketsLost = 0.0;
/**
 * Factor for the definition of rotational limits using the Cartesian Pose
 * interface
 */
...
constexpr std::array<double, 7> kMaxTorqueRate{
    {1000 - kLimitEps, 1000 - kLimitEps, 1000 - kLimitEps, 1000 - kLimitEps,
     1000 - kLimitEps, 1000 - kLimitEps, 1000 - kLimitEps}};
/**
 * Maximum joint jerk
 */
...
constexpr std::array<double, 7> kMaxJointAcceleration{
    {10.0000 - kLimitEps, 10.0000 - kLimitEps, 10.0000 - kLimitEps,
     ...
     10.0000 - kLimitEps}};
/**
 * Tolerance value for joint velocity limits to deal with numerical errors and
 * data losses.
 */
...
constexpr double kMaxTranslationalVelocity =
    3.0000 - kLimitEps -
    kTolNumberPacketsLost * kDeltaT * kMaxTranslationalAcceleration;
/**
...
constexpr double kMaxRotationalVelocity =
    2.5000 - kLimitEps -
    kTolNumberPacketsLost * kDeltaT * kMaxRotationalAcceleration;
/**
 * Maximum elbow jerk
 */
...
constexpr double kMaxElbowVelocity =
    1.5000 - kLimitEps -
    kTolNumberPacketsLost * kDeltaT * kMaxElbowAcceleration;
...
inline std::array<double, 7> computeUpperLimitsJointVelocity(
  ...
  return std::array<double, 7>{
      std::min(2.62,
               std::max(0.0, -0.30 + std::sqrt(std::max(
                                         0.0, 12.0 * (2.75010 - q[0]))))) -
          kJointVelocityLimitsTolerance[0],
      std::min(2.62,
               std::max(0.0, -0.20 + std::sqrt(std::max(
                                         0.0, 5.17 * (1.79180 - q[1]))))) -
          kJointVelocityLimitsTolerance[1],
      std::min(2.62,
               std::max(0.0, -0.20 + std::sqrt(std::max(
                                         0.0, 7.00 * (2.90650 - q[2]))))) -
      ...
      std::min(5.26,
               std::max(0.0, -0.35 + std::sqrt(std::max(
                                         0.0, 34.0 * (2.81010 - q[4]))))) -
          kJointVelocityLimitsTolerance[4],
      std::min(4.18,
               std::max(0.0, -0.35 + std::sqrt(std::max(
                                         0.0, 11.0 * (4.52050 - q[5]))))) -
          kJointVelocityLimitsTolerance[5],
      std::min(5.26,
               std::max(0.0, -0.35 + std::sqrt(std::max(
                                         0.0, 34.0 * (3.01960 - q[6]))))) -
...
inline std::array<double, 7> computeLowerLimitsJointVelocity(
  ...
  return std::array<double, 7>{
      std::max(-2.62,
               std::min(0.0, 0.30 - std::sqrt(std::max(
                                        0.0, 12.0 * (2.750100 + q[0]))))) +
          kJointVelocityLimitsTolerance[0],
      std::max(-2.62,
               std::min(0.0, 0.20 - std::sqrt(std::max(
                                        0.0, 5.17 * (1.791800 + q[1]))))) +
          kJointVelocityLimitsTolerance[1],
      std::max(-2.62,
               std::min(0.0, 0.20 - std::sqrt(std::max(
                                        0.0, 7.00 * (2.906500 + q[2]))))) +
          kJointVelocityLimitsTolerance[2],
      std::max(-2.62,
               std::min(0.0, 0.30 - std::sqrt(std::max(
                                        0.0, 8.00 * (3.048100 + q[3]))))) +
          kJointVelocityLimitsTolerance[3],
      std::max(-5.26,
               std::min(0.0, 0.35 - std::sqrt(std::max(
                                        0.0, 34.0 * (2.810100 + q[4]))))) +
          kJointVelocityLimitsTolerance[4],
      std::max(-4.18,
               std::min(0.0, 0.35 - std::sqrt(std::max(
                                        0.0, 11.0 * (-0.54092 + q[5]))))) +
          kJointVelocityLimitsTolerance[5],
      std::max(-5.26,
               std::min(0.0, 0.35 - std::sqrt(std::max(
                                        0.0, 34.0 * (3.019600 + q[6]))))) +
          kJointVelocityLimitsTolerance[6],
  };
}

/**
 * Limits the rate of an input vector of per-joint commands considering the
 * maximum allowed time derivatives.
 *
 * @note
 ...
 * @param[in] commanded_velocity Commanded joint velocity of the current time
 * step.
 * @param[in] last_commanded_velocity Commanded joint velocity of the previous
 * time step.
 * @param[in] last_commanded_acceleration Commanded joint acceleration of the
 * previous time step.
 *
 ...
 */
double limitRate(double upper_limits_velocity, double lower_limits_velocity,
 ...
 * @param[in] max_acceleration Maximum allowed acceleration.
 * @param[in] max_jerk Maximum allowed jerk.
 * @param[in] commanded_position Commanded joint position of the current time
 * step.
 * @param[in] last_commanded_position Commanded joint position of the previous
 * time step.
 * @param[in] last_commanded_velocity Commanded joint velocity of the previous
 * time step.
 * @param[in] last_commanded_acceleration Commanded joint acceleration of the
...
double limitRate(double upper_limits_velocity, double lower_limits_velocity,
                 double max_acceleration, double max_jerk,
                 double commanded_position, double last_commanded_position,
 ...
 * @param[in] commanded_velocities Commanded joint velocity of the current time
 * step.
 * @param[in] last_commanded_velocities Commanded joint velocities of the
 * previous time step.
 * @param[in] last_commanded_accelerations Commanded joint accelerations of the
 * previous time step.
 *
 ...
 */
std::array<double, 7> limitRate(
    const std::array<double, 7>& upper_limits_velocity,
 ...
 * @param[in] max_acceleration Per-joint maximum allowed acceleration.
 * @param[in] max_jerk Per-joint maximum allowed jerk.
 * @param[in] commanded_positions Commanded joint positions of the current time
 * step.
 * @param[in] last_commanded_positions Commanded joint positions of the previous
 * time step.
 * @param[in] last_commanded_velocities Commanded joint velocities of the
 * previous time step.
 * @param[in] last_commanded_accelerations Commanded joint accelerations of the
...
/**
 * Limits the rate of a desired Cartesian velocity considering the limits
 * provided.
 *
 * @note
 * FCI filters must be deactivated to work properly.
 ...
 * @param[in] last_O_dP_EE_c Commanded end effector twist of the previous time
 * step.
 * @param[in] last_O_ddP_EE_c Commanded end effector acceleration of the
 * previous time step.
 *
...
std::array<double, 6> limitRate(
    ...
    const std::array<double, 6>&
        O_dP_EE_c,  // NOLINT(readability-identifier-naming)
    const std::array<double, 6>&
        last_O_dP_EE_c,  // NOLINT(readability-identifier-naming)
    const std::array<double, 6>&
        last_O_ddP_EE_c);  // NOLINT(readability-identifier-naming)
...
std::array<double, 16> limitRate(
    ...
    const std::array<double, 16>&
        O_T_EE_c,  // NOLINT(readability-identifier-naming)
    const std::array<double, 16>&
        last_O_T_EE_c,  // NOLINT(readability-identifier-naming)
    const std::array<double, 6>&
        last_O_dP_EE_c,  // NOLINT(readability-identifier-naming)
    const std::array<double, 6>&
        last_O_ddP_EE_c);  // NOLINT(readability-identifier-naming)
