 ...
 * An example showing how to generate a Cartesian velocity motion with an
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    auto callback_control = [=, &time](const agimus_franka::RobotState&,
        ...
        -> agimus_franka::CartesianVelocities {
      time += period.toSec();
      ...
      agimus_franka::CartesianVelocities output = {
          {v_x, 0.0, v_z, 0.0, 0.0, 0.0}};
      if (time >= 2 * time_max) {
        std::cout << std::endl
