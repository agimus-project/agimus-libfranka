...
class RobotImplMock : public Robot::Impl {
 public:
  ...
  RobotImplMock(std::unique_ptr<Network> network, size_t log_size,
  ...
  MOCK_METHOD2(writeOnce, void(const JointPositions& motion_input,
                               const Torques& control_input));
  MOCK_METHOD2(writeOnce, void(const JointVelocities& motion_input,
                               const Torques& control_input));
  MOCK_METHOD2(writeOnce, void(const CartesianPose& motion_input,
                               const Torques& control_input));
  MOCK_METHOD2(writeOnce, void(const CartesianVelocities& motion_input,
                               const Torques& control_input));

  MOCK_METHOD0(readOnce, RobotState());
  MOCK_METHOD2(throwOnMotionError,
               void(const RobotState& robot_state, uint32_t motion_id));
};
